	...
	var validate = $("#signupForm").validate({
		...
		messages: {
			...
			password: {
				...
				minlength: jQuery.format($.i18n.prop("密码格式错误")),
	...
	if(validate.form()){
		if($("#checkBox").attr("checked")){
			...
			$.ajax({
				...
				data: {
					email: $("#email").val(),
					password: md5.MD5($("#password").val()).toUpperCase()
				...
				success: function(data){
					...
					if(data.hasOwnProperty("code")){
						if(data.code == 0){
							//注册成功
							window.location.href = "/parenting_jfinal";
						}else if(data.code == 1){
							//数据库链接失败
							$(".login-error").html($.i18n.prop("数据库连接异常"));
						}else if(data.code == 2){
							//参数传递失败
							$(".login-error").show();
							...
							$("#email").next("label.repeated").text($.i18n.prop("邮箱已被注册"));
							registError.show();
						}else{
							//系统错误
			...
			//勾选隐私政策和服务条款
			$(".login-error").show();
			$(".login-error").html($.i18n.prop("请确定是否同意条款"));
		}
	}
}
