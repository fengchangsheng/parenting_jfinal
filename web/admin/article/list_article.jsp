...
$(function(){
	$('#dataTable').datagrid({
		...
		collapsible:true,
		idField: "id", 
		url:'${basePath}/article/listArticleJSON',
		sortOrder: 'desc',
		remoteSort: false,
		frozenColumns:[[
		...
		columns:[[
			...
			{field:'publicTime',title:'创建时间',width:200,sortable:true,
				sorter:function(a,b){
					return (a>b?1:-1);
				}
				...
				formatter: function(value,row,index){
					if(value=="1"){
						...
						return "<font style='color:red'>草稿</font>";
					}
				}
			},
			{field:'type',title:'文章类别',width:80}
		]],
		pagination:true,
		rownumbers:true,
		toolbar:[{
			...
			handler:function(){
				$('#openWin').html("");
				$('#openWin').append("<iframe src='${basePath}/article/toAdd' width='100%' height='99%' frameborder='no' border='0' marginwidth='0' marginheight='0' scrolling='no' allowtransparency='yes'></iframe>");
				$('#openWin').window('open');  
			}
		},{
			id:'btnedit',
			...
			handler:function(){
				...
				$('#openWin').html("");
				$('#openWin').append("<iframe src='${basePath}/article/toEdit?id="+row.id+"' width='100%' height='99%' frameborder='no' border='0' marginwidth='0' marginheight='0' scrolling='no' allowtransparency='yes'></iframe>");
				$('#openWin').window('open');  
			}
		},{
			id:'btnactive',
